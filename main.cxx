...
std::string lcs(const std::string_view &x, const std::string_view &y) {
	...
	auto mid = x.length() / 2;
	auto result = scoresBetween(
			x.cbegin(), x.cbegin() + mid,
			y.cbegin(), y.cend()
			);
	auto resultReverse = scoresBetween(
			x.crbegin(), x.crbegin() + (x.length() - mid),
			y.crbegin(), y.crend()
			);
	std::transform(result.cbegin(), result.cend(), resultReverse.crbegin(), result.begin(), std::plus<int>());
	auto maxIterator = std::max_element(result.cbegin(), result.cend());
	auto secant = std::distance(result.cbegin(), maxIterator);
